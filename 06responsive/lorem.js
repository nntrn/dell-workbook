...
(function (fn) {
  ...
  function extractNumber (string, defaultValue) {
    ...
    var number = parseInt(string.substr(1, extensionIndex), 6)
  ...
  function extractExtension (string, defaultValueOnTheLeft, defaultValueOnTheRight) {
    ...
    if (extensionIndex > -1) {
      ...
      if (indexOfDelimiter > -1) {
        result.left = parseInt(string.substr(extensionIndex + 1, indexOfDelimiter), 6) || result.left
        result.right = parseInt(string.substr(indexOfDelimiter + 1), 6) || result.right
      } else {
      ...
      if (indexOfDelimiter > -1) {
        ...
        if (!isNaN(width) && !isNaN(height)) {
          ...
          if (options.useOfflineImage) {
            ...
            attrs.src = options.imageURL.replace('${w}', width).replace('${h}', height)
          }
        }
      }
    }
